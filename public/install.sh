...
api_origin="${VSEARCH_API_ORIGIN:-https://search.vectorepoch.com}"
token="${VSEARCH_INSTALL_TOKEN:-}"
label="${VSEARCH_INSTALL_LABEL:-vsearch-agent}"
while [[ $# -gt 0 ]]; do
  case "$1" in
    --token) token="${2:-}"; shift 2 ;;
    --label) label="${2:-vsearch-agent}"; shift 2 ;;
...
export VSEARCH_INSTALL_TOKEN="$token"
export VSEARCH_INSTALL_LABEL="$label"
export VSEARCH_API_ORIGIN="$api_origin"
node <<'NODE'
