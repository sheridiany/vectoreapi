...
func SaveQuotaDataCache() {
	...
	for _, quotaData := range CacheQuotaData {
		...
		DB.Table("quota_data").
			Where("user_id = ? and enterprise_id = ? and username = ? and model_name = ? and created_at = ? and use_group = ? and token_id = ? and channel_id = ? and node_name = ?",
				quotaData.UserID, quotaData.EnterpriseID, quotaData.Username, quotaData.ModelName, quotaData.CreatedAt, quotaData.UseGroup, quotaData.TokenID, quotaData.ChannelID, quotaData.NodeName).
			First(quotaDataDB)
		if quotaDataDB.Id > 0 {
...
func GetQuotaDataByUsername(username string, startTime int64, endTime int64, enterpriseID int) (quotaData []*QuotaData, err error) {
...
func GetAllQuotaDates(startTime int64, endTime int64, username string, enterpriseID int) (quotaData []*QuotaData, err error) {
	if username != "" {
		return GetQuotaDataByUsername(username, startTime, endTime, enterpriseID)
	}
	var quotaDatas []*QuotaData
	query := DB.Table("quota_data").
